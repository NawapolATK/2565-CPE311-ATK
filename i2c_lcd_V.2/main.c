...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
  lcd_init ();//เริ่มต้นใช้ฟังชันก์

  lcd_send_string ("Hell WORLD");//ส่งสติงไป

  HAL_Delay(1000);//Delat1000ms(รอ1วิ)

 // lcd_clear();//เคลียร์หน้าจอ

  lcd_put_cur(1,2);//วางเคอร์เซอร์ที่แถวแรกและคอลัมน์2

  lcd_send_string ("from Phuthanet");
  /* USER CODE END 2 */
