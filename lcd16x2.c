...
bool is8BitsMode = true;
GPIO_TypeDef* PORT_RS_and_E;               // RS and E PORT
uint16_t PIN_RS, PIN_E;                    // RS and E pins
GPIO_TypeDef* PORT_LSB;                    // LSBs D0, D1, D2 and D3 PORT
uint16_t D0_PIN, D1_PIN, D2_PIN, D3_PIN;   // LSBs D0, D1, D2 and D3 pins
GPIO_TypeDef* PORT_MSB;                    // MSBs D5, D6, D7 and D8 PORT
uint16_t D4_PIN, D5_PIN, D6_PIN, D7_PIN;   // MSBs D5, D6, D7 and D8 pins
#define T_CONST   20
uint8_t DisplayControl = 0x0F;
 ...
 */
void lcd16x2_write(uint8_t wbyte)
{
 ...
 * @brief Write command
 */
void lcd16x2_writeCommand(uint8_t cmd)
{
  lcd16x2_rs(false);
  lcd16x2_write(cmd);
}
 ...
 * @brief 4-bits write
 */
void lcd16x2_write4(uint8_t nib)
{
  nib &= 0xF;
  lcd16x2_rs(false);
  //LSB data
